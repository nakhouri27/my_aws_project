...
#5 VPC can be created in 1 region & 200 subnets can be created in one  VPC. Once we create VPC, DHCP,NACL & security group is created automatically.
...
################################################################################
#A DHCP option set is a group of network configurations used by EC2 instances in our VPC to communicate over our virtual network.
...
#It supports IPv4 and IPv6 traffic. It does not cause availability risks or bandwidth constraints on our network traffic.
#An IG is a virtual router that connects a VPC to the Internet. Default VPC is already attached to IG
...
#If a subnet's traffic is routed to an internet gateway is known as public subnet
...
#If a subnet 's traffic does not have route to an internet gateway is known as private subnet. The block size is b/w 16 to 28 netmask.
#The first 4 & last  IP of subnet can be assigned.
#1: Reserved for VPC Router
#2:  Reserved for DNS Server
#3: Reserved for Future use
#4: Reserved for Broadcast address
...
# NAT Gateway
################################################################################
#A NAT gateway is a Network Address Translation (NAT) service. We can use a NAT gateway so that instances in a private subnet can connect to services outside our VPC
...
#We can have upto 200 route tables & 50 route entries per route table. Each subnet must be associated with one route table but one route table can hsve multiple subnets.
